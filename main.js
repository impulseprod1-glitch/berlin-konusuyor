...
const translations = {
  tr: {
    ...
    events_cal_desc: "Berlin'de bu hafta sonu neler var?",
  },
  de: {
...
function renderNews(articles) {
  ...
  const featuredHTML = `
    <article class="news-featured magnetic-card" onclick="openNewsModal(0)">
      <div class="news-img" style="background-image: url('${featured.image || PLACEHOLDER_IMG}')"></div>
      <div class="news-featured-overlay"></div>
      <div class="news-featured-body">
        <div class="news-meta">
          <span class="news-source-tag">${featured.source}</span>
          <span class="news-date">${formatDate(featured.date)}</span>
        </div>
        <h3 class="news-headline-large">${featured.summary_tr || featured.title}</h3>
        <p class="news-excerpt">${featured.description?.substring(0, 150) || ''}${featured.description?.length > 150 ? '...' : ''}</p>
      </div>
    </article>
  `;

  const sideHTML = side.map((a, i) => `
    <article class="news-card glass-panel" onclick="openNewsModal(${i + 1})">
      <div class="news-card-img" style="background-image: url('${a.image || PLACEHOLDER_IMG}')"></div>
      <div class="news-card-content">
        <div class="news-meta">
          <span class="news-source-tag">${a.source}</span>
          <span class="news-date">${formatDate(a.date)}</span>
        </div>
        <h4 class="news-card-title">${a.summary_tr || a.title}</h4>
      </div>
    </article>
  `).join('');
...
// Close modal on click outside (Update existing window.onclick)
const orgOnclick = window.onclick;
window.onclick = (event) => {
  if (orgOnclick) orgOnclick(event); // run existing handler
